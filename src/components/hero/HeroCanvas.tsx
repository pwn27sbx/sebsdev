...
const HeroCanvas = () => {
  ...
  useEffect(() => {
    ...
    const mainBloom = new UnrealBloomPass(new THREE.Vector2(window.innerWidth, window.innerHeight), 0.4, 0.55, 0)
    ...
    const finalPassMaterial = new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        void main(){
          ...
          vec3 halo = vec3(0.0);
          // texture2D(haloTexture, vUv).xyz; // If halo is provided
          gl_FragColor = vec4(bg + flame*uFlameAmt + texture2D(bloomTexture, vUv).xyz + texture2D(torusTexture, vUv).xyz + texture2D(tDiffuse, vUv).xyz + halo, 1.);
        }
      `
    })

    const finalPass = new ShaderPass(finalPassMaterial, 'tDiffuse')
    const finalComposer = new EffectComposer(renderer)
    finalComposer.addPass(renderPass)
    finalComposer.addPass(finalPass)

    // --- Geometry & Material (Flow Wave Points) ---
    ...
    const animate = () => {
      ...
      mat.uniforms.uStream.value = state.stream
      mat.uniforms.uWaveHeight.value = waveHeight * (1 + scrollCurrent * scrollRise)
      ...
      mat.uniforms.uAppear.value = Math.max(0, Math.min(1, (elapsed - 0.2) / 1.4))
  ...
  return (
    <canvas
      ref={canvasRef}
      className="absolute inset-0 w-full h-full pointer-events-none gpu transition-opacity duration-1000"
      aria-hidden="true"
    />
  )
}
