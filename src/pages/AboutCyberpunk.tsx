    ...
    <div className="min-h-screen relative bg-[var(--color-bg-light)] dark:bg-[var(--color-bg-dark)] bg-[linear-gradient(to_right,rgba(0,0,0,0.03)_1px,transparent_1px),linear-gradient(to_bottom,rgba(0,0,0,0.03)_1px,transparent_1px)] dark:bg-[linear-gradient(to_right,rgba(255,255,255,0.05)_1px,transparent_1px),linear-gradient(to_bottom,rgba(255,255,255,0.05)_1px,transparent_1px)] bg-[length:40px_40px] bg-fixed text-[#111] dark:text-white font-sans transition-colors duration-500 overflow-x-hidden pt-24 pb-20 px-6 sm:px-12 md:px-24">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-20 max-w-5xl relative z-30">
        ...
        <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: '-80px' }} transition={{ duration: 0.6, delay: 0.3 }}>
          <h3 className="text-xs uppercase tracking-widest text-secondary font-bold mb-6 flex items-center gap-2">
            <span className="text-primary">&gt;</span> {t('aboutTechs', lang)}
          </h3>
          <div className="flex flex-wrap gap-3">
            {techs.map((tech, i) => (
              <motion.span key={tech} initial={{ opacity: 0, scale: 0.8 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: 0.4 + i * 0.05 }}
                className="px-4 py-2 border border-gray-300 dark:border-gray-800 bg-white/50 dark:bg-black/50 font-mono text-[10px] sm:text-xs tracking-wider text-gray-600 dark:text-gray-400 hover:border-secondary hover:text-secondary hover:bg-secondary/10 transition-colors duration-300 relative group overflow-hidden"
              >
                <div className="absolute left-0 top-0 bottom-0 w-1 bg-secondary scale-y-0 group-hover:scale-y-100 transition-transform duration-300 origin-bottom" />
